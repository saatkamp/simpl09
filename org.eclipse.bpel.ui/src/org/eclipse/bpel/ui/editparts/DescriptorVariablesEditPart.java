...
public class DescriptorVariablesEditPart extends BPELTrayCategoryEditPart {
  ...
  /**
   * Overridden to set the type and create a clone variable on creation of a descriptor variable.
   * 
   * The data type is automatically chosen and the descriptor variable is cloned to a
  ...
  protected Object addEntry() {
    ...
    // set the type of the descriptor variable
    if (!definitions.isEmpty()) {
      ...
      SetDescriptorVariableTypeCommand command = new SetDescriptorVariableTypeCommand(
          variable, newType);
      command.execute();
    }

    // create variable clone of the descriptor variable
  ...
  /**
   * Override to remove the clone variable when a descriptor variable is removed.
   */
  ...
  protected void removeEntry() {
    ...
    for (Iterator<EditPart> iter = selectedParts.iterator(); iter.hasNext();) {
      ...
      if (part.getParent() == DescriptorVariablesEditPart.this) {
        condemned.add(part);
      }
    }

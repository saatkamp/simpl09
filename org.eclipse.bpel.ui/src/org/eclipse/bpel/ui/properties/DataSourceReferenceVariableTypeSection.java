...
import org.eclipse.bpel.ui.commands.SetDataSourceReferenceVariableKindCommand;
...
/**
 * DataSourceReferenceVariableTypeSection provides viewing and editing of the type of a BPEL variable
 * (whether that be an XSD type, WSDL message, or built-in simple type).
 */
public class DataSourceReferenceVariableTypeSection extends BPELPropertySection {
	...
	protected DataSourceReferenceVariableTypeSelector variableTypeSelector;
	
	protected Composite parentComposite;
	

	protected boolean isMessageTypeAffected(Notification n) {
		return (n.getFeatureID(DataSourceReferenceVariable.class) == BPELPackage.VARIABLE__MESSAGE_TYPE);
	}
	
	protected boolean isTypeAffected(Notification n) {
		return (n.getFeatureID(DataSourceReferenceVariable.class) == BPELPackage.VARIABLE__TYPE);
	}
	
	protected boolean isElementAffected(Notification n) {
		return (n.getFeatureID(DataSourceReferenceVariable.class) == BPELPackage.VARIABLE__ELEMENT);
	}
	...
	public class DataSourceReferenceVariableTypeCallback implements DataSourceReferenceVariableTypeSelector.Callback {
		...
		public void selectRadioButton(int index) {
			DataSourceReferenceVariable var = (DataSourceReferenceVariable)getInput();
			...
			if ((var.getMessageType() != null) || (var.getType() != null) || (var.getXSDElement()) != null) {
				command.add(new SetDataSourceReferenceVariableTypeCommand(var, (Message)null));
			}
			if ((varExt.getVariableKind() != index)) {
				command.add(new SetDataSourceReferenceVariableKindCommand(varExt, index));
			}
		...
		public void selectXSDType(XSDTypeDefinition xsdType) {
			DataSourceReferenceVariable variable = (DataSourceReferenceVariable)getInput();
	    	Command cmd = new SetDataSourceReferenceVariableTypeCommand(variable, xsdType);
		...
		public void selectXSDElement(XSDElementDeclaration xsdElement) {
			DataSourceReferenceVariable variable = (DataSourceReferenceVariable)getInput();
	    	Command cmd = new SetDataSourceReferenceVariableTypeCommand(variable, xsdElement);
			getCommandFramework().execute(wrapInShowContextCommand(cmd));
		}
		...
		public void selectMessageType(Message message) {
			DataSourceReferenceVariable variable = (DataSourceReferenceVariable)getInput();
			Command cmd = new SetDataSourceReferenceVariableTypeCommand(variable, message);
			getCommandFramework().execute(wrapInShowContextCommand(cmd));
		}
	}
	...
	protected void createClient(Composite parent) {
		...
		variableTypeSelector = new DataSourceReferenceVariableTypeSelector(composite, SWT.NONE, getBPELEditor(),
			fWidgetFactory, new DataSourceReferenceVariableTypeCallback(), true);
	...
	public void updateVariableTypeSelector() {
		variableTypeSelector.setVariable((DataSourceReferenceVariable)getInput());
		updateMarkers();
	}

 ...
 */
public class DataSourceReferenceVariableSelectorDialog extends SelectionAndCreationDialog {

	protected static class TypeLabelProvider extends LabelProvider {
		@Override
		public String getText(Object element) {
			if (element instanceof DataSourceReferenceVariable) {
				DataSourceReferenceVariable variable = (DataSourceReferenceVariable)element;
	...
	protected DataSourceReferenceVariable[] variables;
	 ...
	 */
	public DataSourceReferenceVariableSelectorDialog(Shell parent, EObject modelObject) {
		...
		refreshDataSourceReferenceVariables();
	}
	
	protected void refreshDataSourceReferenceVariables() {
		variables = BPELUtil.getVisibleDataSourceReferenceVariables(modelObject);
	...
	protected void handleNewButtonPushed() {
		...
		NameDialog nameDialog = new NameDialog(getShell(), "New DataSourceReferenceVariable", "DataSourceReferenceVariable Name", 
...
//			command.add(new SetVariableTypeCommand(variable, variableType));
	 ...
	 */
	protected DataSourceReferenceVariable getDataSourceReferenceVariable(String name) {
		DataSourceReferenceVariable result = (DataSourceReferenceVariable)nameToVariable.get(name);
	 ...
	 */
	public DataSourceReferenceVariable getDataSourceReferenceVariable() {
		if (getResult().length > 0) {
			return (DataSourceReferenceVariable)getResult()[0];
		}
		return null;
	}

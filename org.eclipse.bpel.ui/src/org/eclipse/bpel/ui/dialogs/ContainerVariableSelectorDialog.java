 ...
 */
public class ContainerVariableSelectorDialog extends SelectionAndCreationDialog {

	protected static class TypeLabelProvider extends LabelProvider {
		@Override
		public String getText(Object element) {
			if (element instanceof ContainerVariable) {
				ContainerVariable variable = (ContainerVariable)element;
	...
	protected ContainerVariable[] variables;
	 ...
	 */
	public ContainerVariableSelectorDialog(Shell parent, EObject modelObject) {
		...
		refreshContainerVariables();
	}
	
	protected void refreshContainerVariables() {
		variables = BPELUtil.getVisibleContainerVariables(modelObject);
	...
	protected void handleNewButtonPushed() {
		...
		NameDialog nameDialog = new NameDialog(getShell(), "New ContainerVariable", "ContainerVariable Name", 
...
//			command.add(new SetVariableTypeCommand(variable, variableType));
	 ...
	 */
	protected ContainerVariable getContainerVariable(String name) {
		ContainerVariable result = (ContainerVariable)nameToVariable.get(name);
	 ...
	 */
	public ContainerVariable getContainerVariable() {
		if (getResult().length > 0) {
			return (ContainerVariable)getResult()[0];
		}
		return null;
	}

...
public class BPELWriter {
	...
	// SIMPL Methods
	protected Element referenceVariables2XML(ReferenceVariables variables) {
		// If there are no variables then skip creating Element
		if (variables.getChildren().isEmpty())
			return null;

		Element variablesElement = createBPELElement("referenceVariables");

		for (Object next : variables.getChildren()) {
			ReferenceVariable variable = (ReferenceVariable) next;
			variablesElement.appendChild(referenceVariable2XML(variable));
		}
	...
	protected Element referenceVariable2XML(ReferenceVariable variable) {
		Element variableElement = createBPELElement("referenceVariable");
		...
		if (variable.getValueType() != null) {
			XSDTypeDefinition type = variable.getValueType();
			QName qname = new QName(type.getTargetNamespace(), type.getName());
			variableElement
					.setAttribute("valueType", qNameToString(variable, qname));
		}

		// PartnerLink
		if (variable.getPartnerLink() != null) {
			variableElement
					.setAttribute("partnerLink", variable.getPartnerLink().getName());
		}

		// ReferenceType
		if (variable.getReferenceType() != null) {
			variableElement
				.setAttribute("referenceType", variable.getReferenceType().toString());
		}

		// Period
		if (variable.getPeriod() != -1){
			variableElement
				.setAttribute("period", String.valueOf(variable.getPeriod()));
		}

...
public class SWTApp {
	...
	private void creatTreeMenu() {
		...
		final TreeItem item1a = new TreeItem(item1, SWT.NONE);
		item1a.setText("Authentification data");

		TreeItem item2 = new TreeItem(tree, SWT.NONE);
		item2.setText("Auditing");
		final TreeItem item2a = new TreeItem(item2, SWT.NONE);
		item2a.setText("General");
		
		for (String s : getTreeItems()){
	...
	private static final String TREE_ITEM_ID = "org.eclipse.simpl.adminConsole.treeItem";
	
	private java.util.List<String> getTreeItems() {
		java.util.List<String> treeItems = new ArrayList<String>();
		try {
			IConfigurationElement[] config = Platform.getExtensionRegistry().
					getConfigurationElementsFor(TREE_ITEM_ID);

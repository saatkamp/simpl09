 ...
 * <b>Description:</b>Uses a UDDI client to respond to the UDDI registry that is
 * defined in the unspecified data source information.<br>
...
public class StrategyServiceImpl implements StrategyService {
	...
	public DataSource findDataSource(DataSource dataSource) {
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("UserName of incoming ds: "
			...
			logger.debug("Strategy of incoming ds: " + dataSource.getLateBinding().getStrategy().toString());
			logger.debug("UDDI address of incoming ds: " + dataSource.getLateBinding().getUddiAddress());
		}
		...
		if (hasDsPolicy) {
			...
			dsPolicyObj = PolicyEngine.getPolicy(new ByteArrayInputStream(
					dataSource.getLateBinding().getPolicy().getBytes()));
			...
			case FIRST_FIND:
				...
				for (UddiDataSource uddiDataSource : dataSources) {
					...
					if (comparePolicy) {
						...
						try {
							...
							wsPolicyObj = PolicyEngine.getPolicy(policyURL
									.openConnection().getInputStream());
						} catch (MalformedURLException e) {
							wsPolicyObj = PolicyEngine
									.getPolicy(new ByteArrayInputStream(
											wsPolicy.getBytes()));
						} catch (IOException e) {
							e.printStackTrace();
						}
				...
				if (resultUddiDataSource != null) {
					...
					resultDataSource.setSubType(resultUddiDataSource
							.getSubtype());

					resultDataSource.getLateBinding().setPolicy(
							resultUddiDataSource.getWsPolicy());

					resultDataSource.setAddress(resultUddiDataSource
							.getAddress());
					resultDataSource.setDataFormat(resultUddiDataSource
							.getDataFormat());
					resultDataSource.getAuthentication().setUser(
							resultUddiDataSource.getUsername());
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Policy of result ds: " + resultDataSource.getLateBinding().getPolicy());
			logger.debug("UDDI address of result ds: " + resultDataSource.getLateBinding().getUddiAddress());
		}

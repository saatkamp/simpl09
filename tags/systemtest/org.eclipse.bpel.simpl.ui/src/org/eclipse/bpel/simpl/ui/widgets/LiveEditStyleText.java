...
public class LiveEditStyleText extends StyledText {
	...
	String KOMPLEX_PARAMETER = "KOMPLEX-PARAMETER",
			SIMPLE_PARAMETER = "SIMPLE-PARAMETER";
	...
	/** The keywords. */
	String[] keywords = { "SELECT", "INSERT", "CALL", "INTO", "DROP", "DELETE",
			"WHERE", "IN", "INOUT", "GROUP BY", "<", ">", "=", "LIKE",
			"ORDER BY", "AVG", "MAX", "MIN", "SUM", "COUNT", "ALL", "DISTINCT",
			"UPDATE", "SCHEMA", "TABLE", "CREATE", "FROM" };
	...
	public LiveEditStyleText(Composite theComposite,
			final DMActivityPropertySection objectDMPropertySection) {

		// super(shell, SWT.MULTI | SWT.WRAP | SWT.BORDER | SWT.H_SCROLL |
		// SWT.V_SCROLL);
			...
			public void lineGetStyle(LineStyleEvent event) {
				...
				for (int i = 0; i < wordsOfSyleText.length; i++) {
					...
					if (isEqualToKeyword(typedWord)) {
						...
						// list.add(getHighlightStyle(styledText.getText().length()-lastTypedWord.length(),
						// styledText.getText().length()-1));
						if ((cursor = line.indexOf(typedWord, cursor + 1)) >= 0) {
							list.add(getHighlightStyle(event.lineOffset
									+ cursor, typedWord.length(), KEYWORD));
						}

						// list.add(getHighlightStyle(index,
						// typedWord.length()));
						...
						if ((cursor = line.indexOf(typedWord, cursor + 1)) >= 0) {
							list.add(getHighlightStyle(event.lineOffset
									+ cursor, tmpWord.length(), TABLE_NAME));
						}
						...
						if ((cursor = line.indexOf(typedWord, cursor + 1)) >= 0) {
							list
									.add(getHighlightStyle(event.lineOffset
											+ cursor, typedWord.length(),
											BPEL_VARIABLE));
						}
		...
		this.setText(objectDMPropertySection.getStatement() != null ? objectDMPropertySection
						.getStatement()
						: "");
				...
				for (int i = 0; i < wordsOfSyleText.length; i++) {
					...
					if (isEqualToKeyword(typedWord)) {
						...
						// typedWord.length()));
							...
							|| (isEqualToSimplParameter(typedWord))) {
						System.out.print("is a keyword");
						// list.add(getHighlightStyle(styledText.getText().length()-lastTypedWord.length(),
						// styledText.getText().length()-1));
						if ((cursor = line.indexOf(typedWord, cursor + 1)) >= 0) {
							list.add(getHighlightStyle(event.lineOffset
									+ cursor, typedWord.length(), PARAMETER));
						}
	...
	private boolean isEqualToSimplParameter(String typedWord) {
		...
		// typedWord.charAt(0).getChars(0, typedWord.length()-1, charsOfWord,
		// 0);
		if ((typedWord.startsWith("#")) && (typedWord.endsWith("#"))
				&& (typedWord.length() > 1)) {
			return true;
		}
		return false;

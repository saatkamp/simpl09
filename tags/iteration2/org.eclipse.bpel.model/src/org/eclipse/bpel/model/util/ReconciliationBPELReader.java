...
public class ReconciliationBPELReader extends BPELReader implements
	 ...
	 * object.
	 */
	protected ReferenceVariable xml2ReferenceVariable(ReferenceVariable variable,
			Element variableElement) {
		if (!variableElement.getLocalName().equals("referenceVariable"))
			return null;

		if (variable == null) {
			variable = BPELFactory.eINSTANCE.createReferenceVariable();
			variable.setElement(variableElement);
		}
		...
		if (period != null && period.getSpecified()) {
			variable.setPeriod(Integer.parseInt(variableElement.getAttribute("period")));
		} else {
	...
	protected ReferenceVariables xml2ReferenceVariables(ReferenceVariables variables,
			Element variablesElement) {
		if (!variablesElement.getLocalName().equals("referenceVariables"))
			return null;

		if (variables == null) {
			variables = BPELFactory.eINSTANCE.createReferenceVariables();
			variables.setElement(variablesElement);
		}
		...
		List<Element> childElements = ReconciliationHelper
				.getBPELChildElementsByLocalName(variablesElement, "referenceVariable");
		EList<ReferenceVariable> childrenList = variables.getChildren();
		syncLists(variablesElement, childElements, childrenList, new Creator() {
			public WSDLElement create(Element element) {
				return xml2ReferenceVariable(null, element);
			}
		});

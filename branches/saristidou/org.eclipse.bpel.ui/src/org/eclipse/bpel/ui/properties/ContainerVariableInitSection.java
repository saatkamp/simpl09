...
/**
 * ContainerVariable from-spec section. This is allowed in BPEL 2.0.
 *  
...
public class ContainerVariableInitSection extends BPELPropertySection {
			
	/** The from section */
	CategorySection fFromSection = null;
	
	/** The current copy rule being edited. */
	ContainerVariable fVariable;
	
	Composite copySelectComposite;
	
	
	/**
	 * ContainerVariable initialization section. This is the LHS of the copy rule.
	 * 
	 */
	
	public ContainerVariableInitSection()  {
	...
	protected boolean isFromAffected ( Notification n ) {		
		return n.getFeature() == BPELPackage.eINSTANCE.getContainerVariable_From();		
	}
	 ...
	 *
	 */
	protected void selectCategoriesForInput (ContainerVariable variable) {
	...
	void updateCategorySelection ( CategorySection section, IAssignCategory newCurrent, boolean bVisual) {
		...
		if (bVisual || fVariable.getFrom() == null) {
			...
			if ( section.fCurrent.isCategoryForModel (null) == false ) {
				cmd = new SetCommand( getInput(), BPELFactory.eINSTANCE.createFrom(), BPELPackage.eINSTANCE.getContainerVariable_From() );	
			} else {
				cmd = new SetCommand( getInput(), null , BPELPackage.eINSTANCE.getContainerVariable_From() );

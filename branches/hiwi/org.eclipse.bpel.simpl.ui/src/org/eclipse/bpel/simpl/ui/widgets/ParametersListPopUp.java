		...
		addToStatement.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				statementText.append(" "
						+ listToSearch.getItems()[listToSearch
								.getSelectionIndex()]);
  ...
  private void loadColumsOfTable(String searchedString) {
		...
		for (int i = 0; i < listOfTableObjekts.size(); i++) {
			...
			if (tmpTableObjekt.getTableName().equals(searchedString)) {
				listColumns.setItems((String[]) tmpTableObjekt
						.getListOfColumnsNames().toArray());
			}
	...
	public void loadBPELVariables(
			java.util.List<String> listOfBPELVariablesAsStrings) {

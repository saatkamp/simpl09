...
/**
 * DescriptorVariable from-spec section. This is allowed in BPEL 2.0.
 *  
...
public class DescriptorVariableInitSection extends BPELPropertySection {
			
	/** The from section */
	CategorySection fFromSection = null;
	
	/** The current copy rule being edited. */
	DescriptorVariable fVariable;
	
	Composite copySelectComposite;
	
	
	/**
	 * DescriptorVariable initialization section. This is the LHS of the copy rule.
	 * 
	 */
	
	public DescriptorVariableInitSection()  {
	...
	protected boolean isFromAffected ( Notification n ) {		
		return n.getFeature() == BPELPackage.eINSTANCE.getDescriptorVariable_From();		
	}
	 ...
	 *
	 */
	protected void selectCategoriesForInput (DescriptorVariable variable) {
	...
	void updateCategorySelection ( CategorySection section, IAssignCategory newCurrent, boolean bVisual) {
		...
		if (bVisual || fVariable.getFrom() == null) {
			...
			if ( section.fCurrent.isCategoryForModel (null) == false ) {
				cmd = new SetCommand( getInput(), BPELFactory.eINSTANCE.createFrom(), BPELPackage.eINSTANCE.getDescriptorVariable_From() );	
			} else {
				cmd = new SetCommand( getInput(), null , BPELPackage.eINSTANCE.getDescriptorVariable_From() );

...
//		(new Exception()).printStackTrace();
		...
		} else if (element instanceof ReferenceVariable) {
    ...
    } else if (element instanceof ContainerVariable) {
      reader.xml2ContainerVariable((ContainerVariable)element, changedElement);
    } else if (element instanceof ContainerVariables) {
      reader.xml2ContainerVariables((ContainerVariables)element, changedElement);
    } else if (element instanceof DescriptorVariable) {
      reader.xml2DescriptorVariable((DescriptorVariable)element, changedElement);
    } else if (element instanceof DescriptorVariables) {
      reader.xml2DescriptorVariables((DescriptorVariables)element, changedElement);
...
//			throw new NotImplementedException(element.getClass().toString());
	...
	public void patchDom(EObject child, EObject parent, Node parentElement, EObject before, Node beforeElement) {
		...
		} else if (child instanceof ReferenceVariable) {
				parentElement = patchParentElement(child, parent, parentElement, BPELConstants.ND_REFERENCE_VARIABLES, BPELConstants.ND_REFERENCE_VARIABLE);

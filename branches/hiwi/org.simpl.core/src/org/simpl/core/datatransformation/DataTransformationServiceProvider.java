...
/**
 * <b>Purpose:</b>Provides access to the data transformation services that are registered in the
 * resource management.<br>
 * <b>Description:</b>Data transformation service instances can be retrieved by their supporting data
...
public class DataTransformationServiceProvider {
   ...
   * @return
   */
  public static DataTransformationService getInstance(String fromDataFormat, String toDataFormat) {
    DataTransformationService dataTransformationService = null;
    Set<List<String>> formats = DataTransformationServiceProvider.dataTransformationServices.keySet();

    for (List<String> list : formats) {
      if (list.contains(fromDataFormat) && list.contains(toDataFormat)) {
        dataTransformationService = DataTransformationServiceProvider.dataTransformationServices.get(list);
      }
    }
    ...
    for (String dataFormatConverterClassName : dataFormatConverterMapping.keySet()) {
      ...
      try {
        if (connectorClassNames.contains(connector.getClass().getName())) {
          dataTransformationService = (DataTransformationService) Class.forName(dataFormatConverterClassName)
              .newInstance();
          ...
          if (dataTransformationService.getFromDataConverter() != null
            ...
            if (dataTransformationService.getFromDataConverter().getDataFormat().equals(dataFormat)) {
              supportedConvertDataFormats.add(dataTransformationService.getToDataConverter()
                  .getDataFormat());
            } else if (dataTransformationService.getToDataConverter().getDataFormat()
                .equals(dataFormat)) {
              supportedConvertDataFormats.add(dataTransformationService.getFromDataConverter()
                  .getDataFormat());
            }
          }
        }
  ...
  public static void loadServices() {
    ...
    while (pluginIterator.hasNext()) {
      try {
        dataTransformationServiceInstance = (DataTransformationService) Class.forName(pluginIterator.next())
            .newInstance();

        if (dataTransformationServiceInstance.getToDataConverter() != null
            && dataTransformationServiceInstance.getFromDataConverter() != null) {
          toDataFormat = dataTransformationServiceInstance.getToDataConverter().getDataFormat();
          fromDataFormat = dataTransformationServiceInstance.getFromDataConverter().getDataFormat();

          DataTransformationServiceProvider.dataTransformationServices.put(
              Arrays.asList(toDataFormat, fromDataFormat), dataTransformationServiceInstance);
        }
      } catch (InstantiationException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();

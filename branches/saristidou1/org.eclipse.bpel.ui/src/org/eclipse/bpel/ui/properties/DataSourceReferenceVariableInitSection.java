...
/**
 * DataSourceReferenceVariable from-spec section. This is allowed in BPEL 2.0.
 *  
...
public class DataSourceReferenceVariableInitSection extends BPELPropertySection {
	...
	DataSourceReferenceVariable fVariable;
	
	Composite copySelectComposite;
	
	
	/**
	 * DataSourceReferenceVariable initialization section. This is the LHS of the copy rule.
	 * 
	 */
	...
	protected boolean isFromAffected ( Notification n ) {		
		return n.getFeature() == BPELPackage.eINSTANCE.getDataSourceReferenceVariable_From();		
	}
	 ...
	 *
	 */
	protected void selectCategoriesForInput (DataSourceReferenceVariable variable) {
	...
	void updateCategorySelection ( CategorySection section, IAssignCategory newCurrent, boolean bVisual) {
		...
		if (bVisual || fVariable.getFrom() == null) {
			...
			if ( section.fCurrent.isCategoryForModel (null) == false ) {
				cmd = new SetCommand( getInput(), BPELFactory.eINSTANCE.createFrom(), BPELPackage.eINSTANCE.getDataSourceReferenceVariable_From() );	
			} else {
				cmd = new SetCommand( getInput(), null , BPELPackage.eINSTANCE.getDataSourceReferenceVariable_From() );

...
public class AuthentificationComposite extends AAdminConsoleComposite {
  ...
  public void loadSettings(String parentItem, String item) {
    ...
    settings = SIMPLCommunication.getConnection().load(parentItem, item, "lastSaved");
    ...
    if (!settings.isEmpty()) {
      ...
      System.out.println(lUser + " / " + bUser);
    }

    // Default-Einstellungen aus SIMPL Core DB laden
    settings = SIMPLCommunication.getConnection().load(parentItem, item, "default");
  ...
  public boolean haveSettingsChanged() {
    ...
    System.out.println(bUser + "/" + this.lUser);
    System.out.println(bPassword + "/" + this.lPassword);
    if (!this.bUser.equals(this.lUser) || !this.bPassword.equals(this.lPassword)) {
      changed = true;
    }
  ...
  public void loadSettingsFromBuffer(String settingName) {
    if (settingName.equals("default")) {
      ...
      System.out.println("LOAD FROM Default: " + this.dUser + " | " + this.dPassword);
      ...
      if (settingName.equals("lastSaved")) {
        ...
        System.out.println("LOAD FROM LastSaved: " + this.lUser + " | " + this.lPassword);
        // Buffer-Werte aktualisieren
        this.bPassword = this.lPassword;
        this.bUser = this.lUser;
      } else {

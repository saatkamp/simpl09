...
public class BPELReader implements ErrorHandler {
	 ...
	 *            the EMF resource to construct
	 * @param inputStream
	 *            the input stream to read the BPEL from
	 */
	...
	protected void setPartnerLink(Element activityElement,
		...
		fPass2Runnables.add(new Runnable() {
			public void run() {
				PartnerLink targetPartnerLink = BPELUtils.getPartnerLink(
						eObject, partnerLinkName);
				if (targetPartnerLink == null) {
					targetPartnerLink = new PartnerLinkProxy(getResource()
							.getURI(), partnerLinkName);
				}
	...
	protected void setVariable(Element activityElement, final EObject eObject,
		...
		// We must do this as a post load runnable because the variable might
		// not
		// exist yet.
		fPass2Runnables.add(new Runnable() {
			public void run() {
	...
	protected void setCompensationHandler(Element element, EObject eObject) {
		Element compensationHandlerElement = getBPELChildElementByLocalName(
	...
	protected void setFaultHandler(Element element,
		...
		if (faultHandlerElements.size() > 0) {
			FaultHandler faultHandler = xml2FaultHandler(faultHandlerElements
					.get(0));
	...
	protected void setEventHandler(Element element,
			ExtensibleElement extensibleElement) {
		List<Element> eventHandlerElements = getBPELChildElementsByLocalName(
				element, "eventHandlers");

		if (eventHandlerElements.size() > 0) {
			EventHandler eventHandler = xml2EventHandler(eventHandlerElements
					.get(0));
	...
	protected void setStandardAttributes(Element activityElement,
		...
		if (suppressJoinFailure != null && suppressJoinFailure.getSpecified())
			activity.setSuppressJoinFailure(new Boolean(suppressJoinFailure
					.getValue().equals("yes")));
	}
	...
	protected Process xml2Process(Element processElement) {
		...
		for (Element e : getBPELChildElementsByLocalName(processElement,
				"import")) {
		...
		// Handle ReferenceVariables Element
		Element referenceVariablesElement = getBPELChildElementByLocalName(
				processElement, "referenceVariables");
    ...
    if (dataSourceReferenceVariablesElement != null)
      process.setDataSourceReferenceVariables(xml2DataSourceReferenceVariables(dataSourceReferenceVariablesElement));
		...
		if (correlationSetsElement != null)
			process
					.setCorrelationSets(xml2CorrelationSets(correlationSetsElement));
		...
		if (messageExchangesElements != null)
			process
					.setMessageExchanges(xml2MessageExchanges(messageExchangesElements));
	...
	public CorrelationSets xml2CorrelationSets(Element correlationSetsElement) {
		...
		CorrelationSets correlationSets = BPELFactory.eINSTANCE
	...
	protected FromParts xml2FromParts(Element fromPartsElement) {
		...
		for (Element e : getBPELChildElementsByLocalName(fromPartsElement,
				"fromPart")) {
	...
	protected ToParts xml2ToParts(Element toPartsElement) {
		...
		for (Element e : getBPELChildElementsByLocalName(toPartsElement,
				"toPart")) {
	...
	public MessageExchanges xml2MessageExchanges(Element messageExchangesElement) {
		...
		MessageExchanges messageExchanges = BPELFactory.eINSTANCE
	...
	protected Extension xml2Extension(Element extensionElement) {
		...
		if (extensionElement.hasAttribute("mustUnderstand"))
			extension.setMustUnderstand(new Boolean(extensionElement
					.getAttribute("mustUnderstand").equals("yes")));
	...
	protected Variable xml2Variable(Element variableElement) {
		...
		// from-spec
		Element fromElement = getBPELChildElementByLocalName(variableElement,
				"from");
		if (fromElement != null) {
	...
	protected FaultHandler xml2FaultHandler(Element faultHandlerElement) {
		...
		for (Element e : getBPELChildElementsByLocalName(faultHandlerElement,
				"catch")) {
	...
	protected Activity xml2Scope(Element scopeElement) {
		...
		if (exitOnStandardFault != null && exitOnStandardFault.getSpecified())
			scope.setExitOnStandardFault(new Boolean(exitOnStandardFault
					.getValue().equals("yes")));
	...
	protected OnMessage xml2OnMessage(Element onMessageElement) {
		...
		// Set fromParts
		Element fromPartsElement = getBPELChildElementByLocalName(
				onMessageElement, "fromParts");
	...
	protected OnEvent xml2OnEvent(Element onEventElement) {
		...
		// Set messageExchange
		setMessageExchange(onEventElement, onEvent, BPELPackage.eINSTANCE
	...
	protected TerminationHandler xml2TerminationHandler(
		...
		TerminationHandler terminationHandler = BPELFactory.eINSTANCE
		...
		terminationHandler
				.setActivity(getChildActivity(terminationHandlerElement));
	...
	protected Expression xml2Expression(Element expressionElement,
		...
		if (expressionElement.hasAttribute("opaque")) {
			expression.setOpaque(new Boolean(expressionElement.getAttribute(
					"opaque").equals("yes")));
		}
	...
	protected From xml2From(From from, Element fromElement) {
		...
		Element serviceRefElement = getBPELChildElementByLocalName(fromElement,
				"service-ref");
		Element literalElement = getBPELChildElementByLocalName(fromElement,
				"literal");
		...
		if (literalElement != null) {
			...
			if (fromElement.hasAttribute("expressionLanguage")) {
				expressionObject.setExpressionLanguage(fromElement
						.getAttribute("expressionLanguage"));
			}
	...
	protected Activity xml2Invoke(Element invokeElement) {
		...
		Element toPartsElement = getBPELChildElementByLocalName(invokeElement,
				"toParts");
	...
	protected Activity xml2Reply(Element replyElement) {
		...
		Element toPartsElement = getBPELChildElementByLocalName(replyElement,
				"toParts");
		if (toPartsElement != null) {
			ToParts toParts = xml2ToParts(toPartsElement);
		...
		// Set messageExchange
		setMessageExchange(replyElement, reply, BPELPackage.eINSTANCE
	...
	protected Activity xml2Receive(Element receiveElement) {
		...
		if (receiveElement.hasAttribute("createInstance")) {
			String createInstance = receiveElement
					.getAttribute("createInstance");
			receive
					.setCreateInstance(new Boolean(createInstance.equals("yes")));
		}

		Element fromPartsElement = getBPELChildElementByLocalName(
				receiveElement, "fromParts");
		if (fromPartsElement != null) {
			FromParts fromParts = xml2FromParts(fromPartsElement);
		...
		// Set messageExchange
		setMessageExchange(receiveElement, receive, BPELPackage.eINSTANCE
		...
		fPass2Runnables.add(new Runnable() {
			public void run() {
				...
				if (targetMessageExchange == null) {
					targetMessageExchange = new MessageExchangeProxy(
							getResource().getURI(), messageExchangeName);
				}
	...
	protected Activity xml2ForEach(Element forEachElement) {
		...
		if (forEachElement.hasAttribute("parallel")) {
			forEach.setParallel(forEachElement.getAttribute("parallel").equals(
					"yes"));
		}
	...
	protected CompletionCondition xml2CompletionCondition(
		...
		CompletionCondition completionCondition = BPELFactory.eINSTANCE
				.createCompletionCondition();
		completionCondition.setElement(completionConditionElement);

		// Set branches element
		Element branchesElement = getBPELChildElementByLocalName(
				completionConditionElement, "branches");
	...
	protected Branches xml2Branches(Element branchesElement) {
		...
		if (branchesElement.hasAttribute("successfulBranchesOnly"))
			branches.setCountCompletedBranchesOnly(new Boolean(branchesElement
	...
	public boolean isAbstractProcess(Element processElement) {
		...
		if (processElement != null) {
			Map<String, String> nsMap = this
					.getAllNamespacesForElement(processElement);
	...
	/**
	 * Converts an XML referenceVariable element to a BPEL ReferenceVariable
	 * object.
	 */
	protected ReferenceVariable xml2ReferenceVariable(Element variableElement) {
		if (!variableElement.getLocalName().equals("referenceVariable"))
			return null;

		ReferenceVariable variable = BPELFactory.eINSTANCE
		...
		// Set value type
		if (variableElement.hasAttribute("valueType")) {
			QName qName = BPELUtils.createAttributeValue(variableElement,
					"valueType");
			XSDTypeDefinition type = new XSDTypeDefinitionProxy(getResource()
					.getURI(), qName);
		...
		// Set PartnerLink
		Attr partnerLink = variableElement.getAttributeNode("partnerLink");

		if (partnerLink != null && partnerLink.getSpecified()) {
			setPartnerLink(variableElement, variable, BPELPackage.eINSTANCE
					.getReferenceVariable_PartnerLink());
		} else {
			variable.setPartnerLink(null);
		}

		// Set ReferenceType
		Attr pattern = variableElement.getAttributeNode("referenceType");

		if (pattern != null && pattern.getSpecified()) {
			if (pattern.getValue().equals("onInstantiation"))
				variable.setReferenceType(ReferenceType.ON_INSTANTIATION);
			else if (pattern.getValue().equals("fresh"))
				variable.setReferenceType(ReferenceType.FRESH);
			else
				// Default-Value
				variable.setReferenceType(ReferenceType.ON_INSTANTIATION);
		}
		...
		// Set Period
		Attr period = variableElement.getAttributeNode("period");

		if (period != null && period.getSpecified()) {
			variable.setPeriod(Integer.parseInt(variableElement
					.getAttribute("period")));
		} else {
			variable.setPeriod(-1);
		}
	...
	public ReferenceVariables xml2ReferenceVariables(Element variablesElement) {
		if (!variablesElement.getLocalName().equals("referenceVariables"))
			return null;

		ReferenceVariables variables = BPELFactory.eINSTANCE
		...
		for (Element e : getBPELChildElementsByLocalName(variablesElement,
				"referenceVariable")) {
			variables.getChildren().add(xml2ReferenceVariable(e));
		}
		xml2ExtensibleElement(variables, variablesElement);
		...
		// children
		// JM: What is this supposed to accomplish?
		List<ReferenceVariable> toBeMoved = new BasicEList<ReferenceVariable>();
		for (Object next : variables.getExtensibilityElements()) {
			if (next instanceof ReferenceVariable) {
				toBeMoved.add((ReferenceVariable) next);
			}
		}

		List<?> extensibility = variables.getExtensibilityElements();
		List<ReferenceVariable> children = variables.getChildren();
		for (ReferenceVariable element : toBeMoved) {
			extensibility.remove(element);
			children.add(element);
		}

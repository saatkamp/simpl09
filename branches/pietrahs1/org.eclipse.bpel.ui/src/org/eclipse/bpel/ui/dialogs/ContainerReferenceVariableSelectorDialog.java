 ...
 */
public class ContainerReferenceVariableSelectorDialog extends SelectionAndCreationDialog {

	protected static class TypeLabelProvider extends LabelProvider {
		@Override
		public String getText(Object element) {
			if (element instanceof ContainerReferenceVariable) {
				ContainerReferenceVariable variable = (ContainerReferenceVariable)element;
	...
	protected ContainerReferenceVariable[] variables;
	 ...
	 */
	public ContainerReferenceVariableSelectorDialog(Shell parent, EObject modelObject) {
		...
		refreshContainerReferenceVariables();
	}
	
	protected void refreshContainerReferenceVariables() {
		variables = BPELUtil.getVisibleContainerReferenceVariables(modelObject);
	...
	protected void handleNewButtonPushed() {
		...
		NameDialog nameDialog = new NameDialog(getShell(), "New ContainerReferenceVariable", "ContainerReferenceVariable Name", 
...
//			command.add(new SetVariableTypeCommand(variable, variableType));
	 ...
	 */
	protected ContainerReferenceVariable getContainerReferenceVariable(String name) {
		ContainerReferenceVariable result = (ContainerReferenceVariable)nameToVariable.get(name);
	 ...
	 */
	public ContainerReferenceVariable getContainerReferenceVariable() {
		if (getResult().length > 0) {
			return (ContainerReferenceVariable)getResult()[0];
		}
		return null;
	}

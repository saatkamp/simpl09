...
public class MetaDataXMLParser {
	...
	ArrayList<DBTable> listOfTableObjekts = new ArrayList<DBTable>();
	ArrayList<DBSchema> listOfSchemaObjects = new ArrayList<DBSchema>();
	DBTable tableObjekt = null;
	DBSchema schemaObject = null;
	...
	public ArrayList<DBTable> loadTablesFromDB(DataSource dataSource) {
		...
		try {
			...
			// .
			Element rootElementOfDSO = document.getDocumentElement();// =DSO
																		// Element;
					...
					if (nl.item(i).getNodeName() == "table") {
						...
						if (dsoXMLElement.getTagName() == "table") {
							...
							if (dsoXMLElement.hasChildNodes()) {
								...
								tableObjekt
										.setListOfColumnsNames(listOfColumnsNames);
								// add it to list
							}
						}
					}
	...
	/**
	 * for parsing the FileSystem Elements from document. parsing the XML
	 * elements into four lists of FileSystemElements
	 * listOfFileSysElements_command listOfFileSysElements_file
	 * listOfFileSysElements_drive listOfFileSysElements_folder
	 * 
	...
	public void loadFileSystemElementsFromDB(DataSource dataSource) {
		...
		try {
			...
			NodeList commandsNodeList = rootElementOfDSO
					.getElementsByTagName("command");
			NodeList driveNodeList = rootElementOfDSO
					.getElementsByTagName("drive");
			NodeList fileNodeList = rootElementOfDSO
					.getElementsByTagName("file");
			NodeList folderNodeList = rootElementOfDSO
			...
			if (commandsNodeList != null && commandsNodeList.getLength() > 0) {
				for (int i = 0; i < commandsNodeList.getLength(); i++) {
					...
					if (commandsNodeList.item(i).getNodeName() == "command") {
						Element dsoXMLElement = (Element) commandsNodeList
								.item(i);
						if (dsoXMLElement.getTagName() == "command") {
							fileSysObjekt = new FileSystemElement();
							fileSysObjekt.setCommandName(dsoXMLElement
									.getAttribute("name"));
							fileSysObjekt.setCommandDescription(dsoXMLElement
				...
				for (int i = 0; i < driveNodeList.getLength(); i++) {
					...
					if (driveNodeList.item(i).getNodeName() == "drive") {
						...
						if (dsoXMLElement.getTagName() == "drive") {
							fileSysObjekt = new FileSystemElement();
							fileSysObjekt.setDriveName(dsoXMLElement
									.getAttribute("name"));
							fileSysObjekt.setDrive_info("Letter:	"
									+ dsoXMLElement.getAttribute("letter")
									+ "\r"
									+ "Total space:	"
									+ dsoXMLElement.getAttribute("totalSpace")
									+ "\r"
									+ "Free space:	"
									+ dsoXMLElement.getAttribute("freeSpace")
									+ "\r"
									+ "Useable space:	"
									+ dsoXMLElement
											.getAttribute("useableSpace")
									+ "\r" + "Writable:	"
									+ dsoXMLElement.getAttribute("writable")
									+ "\r" + "Readable:	"
									+ dsoXMLElement.getAttribute("readable")
									+ "\r" + "Executable:	"
									+ dsoXMLElement.getAttribute("executable")
									+ "\r" + "Type:	"
									+ dsoXMLElement.getAttribute("type"));
						}
					}
				...
				for (int i = 0; i < fileNodeList.getLength(); i++) {
					...
					if (fileNodeList.item(i).getNodeName() == "file") {
						...
						if (dsoXMLElement.getTagName() == "file") {
							fileSysObjekt = new FileSystemElement();
							fileSysObjekt.setFileName(dsoXMLElement
									.getAttribute("name"));
						}
					}
				...
				for (int i = 0; i < folderNodeList.getLength(); i++) {
					...
					if (fileNodeList.item(i).getNodeName() == "folder") {
						...
						if (dsoXMLElement.getTagName() == "folder") {
							fileSysObjekt = new FileSystemElement();
							fileSysObjekt.setFolderName(dsoXMLElement
									.getAttribute("name"));
						}
					}
					listOfFileSysElements_file.add(fileSysObjekt);
	...
	public ArrayList<DBSchema> loadSchemasFromDB(DataSource dataSource) {
		...
		try {
			...
			Element rootElementOfSDO = document.getDocumentElement();
			NodeList nl = rootElementOfSDO.getElementsByTagName("schema");
			if (nl != null && nl.getLength() > 0) {
				for (int i = 0; i < nl.getLength(); i++) {

					// get the child element
					if (nl.item(i).getNodeName() == "schema") {

						Element sdoXMLElement = (Element) nl.item(i);
						if (sdoXMLElement.getTagName() == "schema") {

							schemaObject = new DBSchema();

							schemaObject.setSchemaName(sdoXMLElement
									.getAttribute("name"));
							if (sdoXMLElement.hasChildNodes()) {

								NodeList tableNodesList = sdoXMLElement
										.getChildNodes();

								ArrayList<String> listOfTableNames = new ArrayList<String>();
								Element tableXMLElement;
								for (int j = 0; j < tableNodesList.getLength(); j++) {
									// get the child element
									if (tableNodesList.item(j).getNodeName() == "table") {
										
										tableXMLElement = (Element) tableNodesList
												.item(j);
										listOfTableNames.add(tableXMLElement
												.getAttribute("name"));
									}
								}
								schemaObject
										.setListOfTableNames(listOfTableNames);
							}
						}
					}
					listOfSchemaObjects.add(schemaObject);
				}

...
public class ValidateImpl extends ActivityImpl implements Validate {
  ...
  public EList<Variable> getVariables() {
    if (variables == null) {
      variables = new EObjectResolvingEList<Variable>(Variable.class, this,
          BPELPackage.VALIDATE__VARIABLES);
    }
    return variables;
  }
  ...
  protected void changeReference(EReference reference) {
    if (reference != null && reference.getFeatureID() == BPELPackage.VALIDATE__VARIABLES
        && !isReconciling) {
      String varAttribute = element.getAttribute(BPELConstants.AT_VARIABLES);
      if (variables == null || variables.size() == 0) {
        ReconciliationHelper.replaceAttribute(this, BPELConstants.AT_VARIABLES,
            (String) null);
      } else {
